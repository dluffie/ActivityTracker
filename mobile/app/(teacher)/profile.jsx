...
export default function TeacherProfile() {
    ...
    const infoItems = [
        ...
        { icon: 'school-outline', label: 'Department', value: user?.branch || user?.department },
        { icon: 'code-working-outline', label: 'Staff ID', value: user?.staffId || user?.registrationNumber },
    ];
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                ...
                <View style={styles.infoCard}>
                    {infoItems.map((item, idx) => (
                        <View key={item.label} style={[styles.infoRow, idx < infoItems.length - 1 && styles.infoRowBorder]}>
                ...
                {/* Subscribed Classes */}
                {user?.subscribedClasses?.length > 0 && (
                    <View style={styles.classesCard}>
                        <Text style={styles.classesTitle}>Subscribed Classes</Text>
                        <View style={styles.classesList}>
                            {user.subscribedClasses.map((cls, idx) => (
                                <View key={idx} style={styles.classChip}>
                                    <Ionicons name="school" size={14} color={colors.primary} />
                                    <Text style={styles.classChipText}>{typeof cls === 'string' ? cls : `${cls.branch} - Sem ${cls.semester}`}</Text>
                                </View>
                            ))}
                        </View>
                    </View>
...
const getStyles = (colors) => StyleSheet.create({
    ...
    classesTitle: { fontSize: 15, fontWeight: '600', color: colors.textPrimary, marginBottom: SPACING.md },
    classesList: { flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.sm },
    classChip: {
        flexDirection: 'row', alignItems: 'center', gap: 4,
        paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm,
        backgroundColor: colors.primaryBg, borderRadius: RADIUS.full, borderWidth: 1, borderColor: colors.primaryBorder,
    },
